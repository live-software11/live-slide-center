...
| **C** Smoke test secrets | `apps/web/scripts/smoke-test-upload.mjs` | Lo script aveva hardcoded: Supabase URL, anon key, admin email `admin.alpha@fieldtest.local`, password `FieldTest!AlphaAdmin2026`, session UUID. Anche se le credenziali erano scoped a un tenant di field test, committarle in repo creava esposizione (history + fork + PR pubbliche). | Rimossi tutti i valori hardcoded. Aggiunti helpers `readArgOrEnv(flagName, envName)` + `requireOrFail(value, hint)`: lo script ora richiede CLI flags `--supabase-url`, `--anon-key`, `--email`, `--password`, `--session-id` OR env vars `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `SC_SMOKE_EMAIL`, `SC_SMOKE_PASSWORD`, `SC_SMOKE_SESSION_ID`. Fail-fast con messaggio chiaro se mancanti. Le credenziali field test restano documentate in `docs/FIELD_TEST_CREDENTIALS.md` (NOT in git, gitignored). |
...
| 024 | Smoke test cloud SOLO via env vars / CLI flags (Sprint X-1)          | Mai hardcodare credenziali in repo, anche se scoped a field test. Le credenziali stanno in `FIELD_TEST_CREDENTIALS.md` (gitignored) |
