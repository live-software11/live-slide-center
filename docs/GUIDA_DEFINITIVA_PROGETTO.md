...
> **Versione:** 3.0.15 — 15 Aprile 2026 (quote tenant in UI: panel storage/eventi per mese e sale/evento; gate creazione; `fetchTenantQuotaRow`; guida §11/§15)
...
**Stima MVP (fasi 1–6, cfr. logica sopra):** in aprile 2026, indicativamente **25–31%** del percorso verso il MVP cloud vendibile: fase **0** completata; fasi **1** e **2** in corso (auth, tenant, `/events`, `/events/:eventId` con sale in lista + insert + **update** nome/tipo + delete con conferma; sessioni/relatori con lista + insert + **update** inline + delete con conferma; **prima UI quote** letta da `tenants` + gate client su nuovo evento / nuova sala; mancano upload TUS effettivo, regia realtime, pairing + Room Player PWA, enforcement quote lato DB su `events`/`rooms`, import CSV, calendario drag&drop); fasi **3–6** non iniziate. Se si considera l’intera roadmap **0–14** con pesi simili per fase, la percentuale lineare sul totale visione prodotto resta indicativamente **14–20%** (molte fasi ancora «Da fare» o premium).
...
**EN:** For **MVP phases 1–6**, the project is roughly **25–31%** complete (foundation + tenant event CRUD with deletes; **rooms**, **sessions**, and **speakers** support inline edits where implemented; **first quotas UX** read from `tenants` plus client-side gates for new events / new rooms; major gaps: full upload/TUS portal, realtime control room, pairing + Room Player PWA, DB-level quota enforcement on `events`/`rooms`, CSV import, drag-and-drop calendar). Across **all roadmap phases 0–14**, a naive equal-weight view is about **14–20%** because many phases are untouched. **Tooling:** without Docker, local Supabase CLI flows stay blocked; §18 checkboxes reflect that. **`database.ts`** stays hand-synced with migrations until `gen types --local` is viable.
