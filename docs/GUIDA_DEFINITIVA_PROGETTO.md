...
> **Versione:** 3.0.18 — 15 Aprile 2026 (sessioni: vista «Per sala» read-only raggruppata per sala + `scheduled_start`; riordino DnD `display_order` invariato; guida §11/§15)
...
| 2     | CRUD Eventi/Sale/Sessioni/Speaker        | **In corso**   | `/events` lista+insert+**UI quote** (storage, eventi/mese per `start_date`, gate creazione); `/events/:eventId` sale (**update** nome/tipo)+quote sale/evento+gate creazione sala; sessioni+relatori (lista+insert+**update** inline+delete conferma; **DnD elenco sessioni** `display_order`; **vista Per sala** read-only raggruppata per sala; link+QR invite, token 90gg; legacy rigenerabile); **import CSV relatori** (`speaker-csv-import`, max 200 righe); restano griglia calendario/timeline interattiva, eventuali import sale/sessioni |
