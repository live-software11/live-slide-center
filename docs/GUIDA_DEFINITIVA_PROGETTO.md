...
> **Versione:** 4.16.0 - 17 Aprile 2026 (**Sprint 8 in-repo chiuso al 100% + PIANO_FINALE chiuso — polish operativo finale (audit log tenant + welcome email + status page pubblica + guida uso interno DHS)**): chiusi gli ultimi 4 polish operativi prima della fase di test sul campo. Il prodotto e' ora **operativamente al 100% per uso interno DHS**, con una superficie pubblica (status page) per stabilire fiducia futura con clienti esterni e una superficie privata (audit log + welcome email + guida operativa) per professionalizzare l'uso interno quotidiano. **Database**: nuova migration `supabase/migrations/20260417150000_sprint8_tenant_audit.sql` con 2 indici composti (`idx_activity_log_tenant_action_created`, `idx_activity_log_tenant_actor_created`) per migliorare le performance dei filtri admin sull'`activity_log` esistente, + nuova RPC `list_tenant_activity(p_from, p_to, p_action, p_actor_id, p_entity_type, p_limit, p_offset)` SECURITY DEFINER `search_path=public` che verifica esplicitamente il ruolo `admin` sul tenant corrente via `auth.jwt()` (errore `forbidden_admin_only` 42501 altrimenti), applica filtri opzionali con `ILIKE %p_action%` per match parziale, cap a 200 per `LIMIT`, ritorna JSONB `{ rows, total, has_more, limit, offset }`, GRANT EXECUTE solo a `authenticated`. **Edge Functions**: nuova `supabase/functions/system-status/index.ts` (`verify_jwt = false` in `config.toml`) con 4 probe parallele via `Promise.all` su Database (HEAD `/rest/v1/?select=`), Auth (GET `/auth/v1/health`), Storage (GET `/storage/v1/bucket` accetta 401 = servizio risponde), Edge (self-ping); soglia degraded `DEGRADED_THRESHOLD_MS=1500`, latency >1500ms → degraded, HTTP 5xx o eccezione fetch → down; aggregazione globale 2+ down → `major_outage`, 1 down o N degraded → `degraded`, tutti operational → `operational`; risposta JSON con `Cache-Control: no-store`. Estesa `supabase/functions/team-invite-accept/index.ts` con helper `dispatchWelcomeEmail()` chiamato fire-and-forget (`void`) dopo creazione utente + marca-invito-consumato; skip silenzioso se `EMAIL_SEND_INTERNAL_SECRET` non configurato (deploy senza Resend non si rompe), POST interno a `/functions/v1/email-send` con `kind: 'welcome'` + `idempotency_key: welcome_${user_id}` deterministica (no duplicati anche su retry), errori loggati ma NON bloccano l'accept. **Frontend**: route lazy `/audit` in `apps/web/src/app/routes.tsx` protetta da `RequireTenantAdmin` con view `apps/web/src/features/audit/AuditView.tsx` (filtri action+entity*type input + from/to datetime-local + bottoni Filtra/Pulisci + paginazione 50/pagina con Previous/Next + total + spinner refresh + expander `<details>` per metadata JSON + toast su errore via `useToast` di Sprint 7) + repository `apps/web/src/features/audit/repository.ts` con tipo `TenantActivityRow` allineato JSONB e funzione `listTenantActivity()` che throws su errore; route lazy `/status` no-auth a livello pubblico con view `apps/web/src/features/status/StatusView.tsx` (header con logo `AppBrandLogo` + back-link login + card stato globale con icona + colore verde/giallo/rosso + descrizione i18n + lista `ServiceRow` con icona stato + nome + stato + latency_ms + sezione help mailto + polling automatico 30s + bottone refresh + `aria-live="polite"`) + repository `apps/web/src/features/status/repository.ts` che chiama Edge Function senza JWT richiesto; aggiornati `apps/web/src/features/auth/LoginView.tsx` con link "Stato del servizio" nel footer (sotto "Password dimenticata") e `apps/web/src/features/settings/SettingsView.tsx` con nuova sezione "Audit log" admin-only (icona `ClipboardList`, intro, link `/audit`) che rispecchia la struttura della sezione "Privacy & GDPR" Sprint 7. **i18n**: ~50 chiavi nuove parity IT/EN su `audit.*`(~17),`status._`(~22 incluse 6 chiavi`status.overall.{operational,degraded,outage}.{title,body}`e 3`status.service.status.{operational,degraded,down}`), `auth.statusPageLink`, `settings.audit_`(3). **Tipi**:`packages/shared/src/types/database.ts`esteso con la nuova RPC`list*tenant_activity`(Args con tutti i filtri opzionali nullable +`p_limit`/`p_offset`opzionali + Returns Json). **Documentazione operativa**: nuovo`docs/Manuali/Guida_Uso_Interno_DHS.md`v1.0 (8 sezioni: 0 Setup una tantum 7 punti checklist con riferimenti ai manuali, 1 Pre-evento T-7/T-1 con creazione evento + verifica stato sistema + setup hardware fisico, 2 Giorno dell'evento con vista regia + durante l'evento + fallback intranet offline, 3 Post-evento con export ZIP + audit log + manutenzione settimanale/mensile, 4 "Cosa fare se..." 5 scenari di troubleshooting comune (licenza, storage, sync, presentazione, sub-evento), 5 Test sul campo checklist 8 verifiche da eseguire 1-2 giorni prima di un evento critico, 6 Numeri utili dashboard Resend/Supabase Studio/GitHub/Live WORKS APP/status page/email supporto, 7 Cosa NON fare 5 anti-pattern operativi, 8 Aggiornamento di questa guida) + indice`docs/Manuali/README.md`aggiornato. **Architettura**: ADR-017 in`.cursor/rules/project-architecture.mdc`documenta scelta esposizione audit log al tenant via RPC SECURITY DEFINER (vs RLS diretta) per garantire filtri/paginazione/check ruolo a livello DB + status page pubblica via Edge Function singola con probe paralleli (vs StatusPage.io a pagamento) per autonomia operativa + welcome email best-effort no-block (vs synchronous) per garantire che l'accept invito non fallisca per problemi di Resend. **PIANO_FINALE_SLIDE_CENTER_v2.md aggiornato a v2.9 + chiuso** con nuova sezione 9 "Sprint 8" (7 sotto-sezioni: 9.1 razionale, 9.2 audit log per admin tenant, 9.3 welcome email automatica, 9.4 status page pubblica, 9.5 guida operativa DHS, 9.6 i18n parity ~50 chiavi, 9.7 DoD), nuova sezione 12 "Chiusura PIANO_FINALE — Test sul campo" (12.1 stato in-repo DONE, 12.2 backlog esterno NON-bloccante per uso interno, 12.3 test sul campo come fase successiva al PIANO con 6 fasi sequenziali, 12.4 definizione di "PIANO chiuso"). **Build verde**:`pnpm install`ok, lint e typecheck`apps/web` verdi, parity i18n IT/EN automatica via script di merge JSON deep. **Risultato operativo**: ogni admin DHS ha visibilita completa sull'attivita del proprio tenant (`/audit`), i clienti esterni avranno visibilita pubblica sullo stato del servizio (`/status` brandizzata Live SLIDE CENTER), gli utenti invitati ricevono automaticamente una welcome email (se Resend configurato — altrimenti silenzioso, no breakage), e Andrea ha una guida operativa quotidiana (`docs/Manuali/Guida_Uso_Interno_DHS.md`) per portare in produzione DHS il prodotto senza dimenticare nessun passo critico. **PIANO_FINALE chiuso**: ulteriori sviluppi dipenderanno solo dai feedback dei test sul campo (sezione 12.3 del PIANO).
**Versione archivio 4.15.0:** 17 Aprile 2026 (**Sprint 7 in-repo chiuso al 100% — operativita interna 100% (GDPR export + email transazionali + cron licenze + warning banner + dashboard reale + pagina privacy)**): chiuso ogni gap operativo che impediva l'uso interno DHS al 100%; il prodotto e' ora **utilizzabile in produzione oggi stesso** anche senza vendita esterna; le azioni burocratiche/sales sono scorporate in `docs/Commerciale/Roadmap_Vendita_Esterna.md`perche' non bloccano l'uso interno. **Database**: nuova migration`supabase/migrations/20260417140000_sprint7_operations.sql`(2 tabelle nuove`email_log`per idempotenza email +`tenant_data_exports`per storico GDPR; bucket Storage privato`tenant-exports`500 MB con RLS prefix tenant; 8 RPC SECURITY DEFINER`search_path=public`con GRANT precisi:`export_tenant_data() → JSONB`admin-only,`tenant_storage_summary() → JSONB`con threshold warning>=80% critical>=95%,`tenant_license_summary() → JSONB`con threshold info<=30 warning<=7 critical<=1 expired,`create_tenant_data_export() → UUID`admin-only rate-limit 5min,`list_tenant_data_exports() → SETOF`ultimi 10,`log_email_sent(...)`service*role-only UPSERT idempotente,`list_tenants_for_license_warning(min,max,kind)`super_admin-only anti-spam idempotente via`email_log.metadata.expires_at_iso`, `expire_old_data_exports() → INT`housekeeping). **3 Edge Functions nuove**:`gdpr-export/`admin-JWT con JSZip costruisce ZIP`manifest.json + tenant-data.json + README.txt + 10 CSV`(users/events/rooms/sessions/speakers/presentations/presentation_versions/local_agents/paired_devices/audit_log_90d) + upload Storage + signed URL 7gg + update record ready/failed;`email-send/`Resend transactional con header`x-internal-secret`timing-safe +`verify_jwt=false`+ 4 template HTML inline IT/EN (welcome, license-expiring, storage-warning, event-published) + idempotency check su`email_log`+ log success/failure via`log_email_sent`; `email-cron-licenses/`daily scan 3 soglie T-30/T-7/T-1 → dispatch a`email-send`con`idempotency_key='{kind}*{tenant*id}*{expires_at}'`+`dry_run`mode + 3 opzioni schedule (GitHub Actions consigliato, cron-job.org, pg_cron). **Frontend**: toast system globale`apps/web/src/components/{ToastProvider.tsx, use-toast.ts}`con max 5 toast contemporaneamente + auto-dismiss 4500ms + a11y`role=status/alert`+ montato in`Providers`sopra`AuthProvider`; warning banner stack `apps/web/src/features/notifications/{repository.ts, hooks/useTenantWarnings.ts, components/TenantWarningBanners.tsx}`montato in`RootLayout`SOPRA`<Outlet />`con polling 5min + dismiss persistente in sessionStorage +`Promise.allSettled`per non bloccare su una RPC che fallisce;`DashboardView.tsx`riscritta con 3 card reali (eventi totali+attivi/setup, storage MB+barra di progresso colorata, licenza data scadenza+giorni rimanenti+tone basato su threshold) + sezione "Prossimo evento" se trovato + vista dedicata super_admin con redirect a`/admin`; route `/settings/privacy`admin-only con`apps/web/src/features/settings/privacy/{PrivacyView.tsx, repository.ts}`(bottone "Esporta dati" con feedback toast success/error/rate-limit + cronologia ultimi 10 export + sezione cancellazione dati + sezione note legali DPO/retention/sub-fornitori); link "Privacy & GDPR" aggiunto in`SettingsView.tsx` admin-only. **i18n**: ~70 chiavi nuove in symmetric su IT/EN (`dashboard.*`con plurali ICU,`notifications.*`con plurali ICU,`privacy.\_`con statuses,`settings.privacy\_`, `common.refresh`); script di merge JSON deep usato per inserire correttamente le chiavi annidate. **config.toml**: aggiunti `[functions.email-send]`+`[functions.email-cron-licenses]`con`verify_jwt = false` per chiamate server-to-server (`gdpr-export`resta default`verify_jwt=true`perche' richiede JWT admin). **Tipi**:`packages/shared/src/types/database.ts`esteso con 2 tabelle nuove + 8 RPC nuove (Args + Returns precisi). **Documentazione operativa**: nuovo`docs/Manuali/Manuale_Email_Resend.md`v1.0 (10 sezioni: account Resend free 3.000/mese vs Pro 50.000/mese, generazione`EMAIL_SEND_INTERNAL_SECRET`>=32 caratteri via PowerShell`RandomNumberGenerator`, secret Supabase Edge `RESEND_API_KEY`/`RESEND_FROM_EMAIL`/`EMAIL_SEND_INTERNAL_SECRET`/`PUBLIC_APP_URL`, `supabase functions deploy gdpr-export email-send email-cron-licenses`, test invio singolo PowerShell con verifica `email_log`, schedulazione cron 3 opzioni con esempi YAML/curl/SQL, test dry-run, anti-spam idempotenza con SQL di verifica, troubleshooting 7 casi tabellari, costi mensili stimati Free vs Pro, estensioni future MJML/webhook/DSAR); nuovo `docs/Commerciale/Roadmap_Vendita_Esterna.md`v1.0 (10 sezioni 47 voci: legale DPA/Privacy/T&C/DPIA/DPO con costi 1.500-3.500€, fiscale P.IVA/VIES/AGCOM/Lemon Squeezy, marketing sito/materiale/video/SEO/social 2.000-4.000€ one-time, pricing piani definitivi con BEP, operations help desk/docs/status page/backup/monitoring 600-1.500€/anno, pipeline prospect/demo/early-adopter/referral, estensioni multi-lingua/mobile/white-label, roadmap 4 fasi 12 mesi, budget complessivo 3.700-7.000€ one-time + 1.090-3.930€/anno, 5 decisioni urgenti pre-primo-cliente). **README cartelle aggiornati** con nuove righe per i 2 documenti. **Architettura**: ADR-016 in`.cursor/rules/project-architecture.mdc`documenta scelta GDPR export server-side via Edge (vs client-side) + Resend per email (vs SMTP self-host) + cron giornaliero esterno (vs pg_cron) per evitare vendor-lock-in e mantenere portabilita. **PIANO_FINALE_SLIDE_CENTER_v2.md aggiornato a v2.8** con nuova sezione 8 "Sprint 7" (8 sotto-sezioni: 8.1 migration DB+RPC, 8.2 Edge gdpr-export, 8.3 Edge email-send Resend, 8.4 Edge email-cron-licenses, 8.5 frontend toast+banner+dashboard+privacy, 8.6 i18n parity ~70 chiavi, 8.7 documentazione operativa, 8.8 DoD). **Manca SOLO azione esterna Andrea (non bloccante per uso interno)**: registrazione Resend + verifica dominio`liveworksapp.com`+ 4 secret Supabase + schedule cron giornaliero (manuale operativo step-by-step). **Risultato**: il prodotto e' utilizzabile in produzione DHS oggi stesso; ogni operatore ha banner che lo informa quando licenza/storage si avvicinano alle soglie; admin ha 1 click per scaricare backup ZIP completo del workspace; appena Andrea configura Resend, le email scadenza licenza partono automaticamente in T-30/T-7/T-1; il dashboard non e' piu' un placeholder vuoto ma una pagina di lavoro reale.
**Versione archivio 4.14.0:** 17 Aprile 2026 (**Sprint 6 in-repo chiuso al 100% — onboarding wizard + demo data + healthcheck + dashboard /admin/health**): chiuso il gap commerciale piu' importante post-MVP: il primo accesso di un cliente non sapeva cosa fare; ora il prodotto si presenta da solo. **Database**: nuova migration`supabase/migrations/20260417130000_onboarding_and_demo_seed.sql`(colonna nullable`tenants.onboarded_at TIMESTAMPTZ`+ 5 RPC SECURITY DEFINER con`SET search_path = public`e`GRANT EXECUTE TO authenticated`): `mark_tenant_onboarded()`admin self →`now()`, `reset_tenant_onboarding()`admin self → NULL (per "Riapri tour" da Settings),`seed_demo_data()`admin/coordinator idempotente (inserisce 1 evento + 2 sale + 3 sessioni + 4 speaker + 5 placeholder presentazioni con marker`settings.demo='true'`, ritorna `{event_id, created: bool}`), `clear_demo_data()`admin self (cancella SOLO eventi con`settings.demo='true'`, cascade su rooms/sessions/speakers/presentations, preserva eventi reali, ritorna count), `tenant_health()`super*admin only (counter aggregati globali:`total_tenants`, `active_events`, `total_users`, `db_size_pretty`). **Tipi shared**: `packages/shared/src/types/database.ts`esteso con`tenants.onboarded_at`Row/Insert/Update + 5 nuove definizioni RPC in`Functions`. **Frontend onboarding**: nuova feature `apps/web/src/features/onboarding/`con`repository.ts`(5 funzioni che mappano le RPC), hook`hooks/useTenantOnboardingStatus.ts`che legge`tenants.onboarded_at`ed espone`{state, isOnboarded, tenantName, refresh}`con effect refactored per rispettare`react-hooks/set-state-in-effect`(microtask via`Promise.resolve().then(...)`invece di setState sync), gate`OnboardingGate.tsx`che verifica`role === 'admin' && tenantId && isOnboarded === false`e fa lazy-load del wizard (zero costo bundle per non-admin),`components/OnboardingWizard.tsx`come modal full-screen 3 step (Welcome con 3 benefit + icone Lucide → Crea evento o Genera demo con form inline + alternativa → Finish con next-step team + agent), skip in qualsiasi step chiama comunque`markTenantOnboarded()`per non riproporre il wizard ad ogni navigazione. **Mounting**:`OnboardingGate`aggiunto a`RootLayout.tsx` fuori dall'`<Outlet />`cosi' resta visibile su qualsiasi route admin. **Settings esteso**:`apps/web/src/features/settings/SettingsView.tsx`ha nuova sezione "Demo & Onboarding" admin-only con 3 azioni`DemoActionRow`(Genera dati demo / Cancella dati demo / Riapri tour), ognuna con stato`idle | busy | done | error`+ feedback inline + disabling durante l'esecuzione. **Empty states migliorati**:`EventsView`mostra card centrata (titolo + body + CTA →`/settings`per generare demo) invece di "Nessun evento" testo nudo;`TeamView`mostra card con bottone "Invita membro" che apre il dialog inviti invece di "Nessun invito" testo nudo. **Healthcheck doppio**: (a)`apps/web/public/healthcheck.json`statico per uptime monitor esterni (UptimeRobot/BetterUptime/Pingdom puntati su`https://app.liveworksapp.com/healthcheck.json`, 200 OK = app raggiungibile, payload con `{status, service, version, release_codename, support}`); (b) pagina `/admin/health` super_admin only in `apps/web/src/features/admin/AdminHealthView.tsx` con 3 sezioni live: Supabase ping (latency RTT su `tenant_health()` con badge colorato verde<500ms / ambra 500-1500ms / rosso>1500ms), Edge Functions ping (`team-invite-accept` + `licensing-sync` con accept-401-as-online perche' richiedono auth), Counter aggregati (`tenant_health()` mostrato in `CounterCard` colorate); bottone "Refresh All" rilancia tutti i ping in parallelo. **Routing**: `apps/web/src/app/routes.tsx` aggiunge route lazy `/admin/health` puntata su `AdminHealthView`; `apps/web/src/app/admin-root-layout.tsx` aggiunge link "Health" in nav admin. **i18n**: ~50 chiavi nuove in symmetric su `packages/shared/src/i18n/locales/it.json` + `en.json` (`onboarding.*` per wizard step/bottoni/messaggi, `settings.demo*` per sezione Demo & Onboarding, `emptyState.*` per events/team, `health.*` per dashboard `/admin/health`). **Architettura**: ADR-015 in `.cursor/rules/project-architecture.mdc` documenta scelta `tenants.onboarded_at` (vs `localStorage` o `app_metadata` JWT) + auto-trigger via `OnboardingGate` + RPC SECURITY DEFINER per gating role-aware + demo data marker `settings.demo='true'` + healthcheck doppio statico+dinamico. **PIANO_FINALE_SLIDE_CENTER_v2.md aggiornato a v2.7** con nuova sezione 7 "Sprint 6" (8 sotto-sezioni: 7.0 obiettivi, 7.1 migration RPC, 7.2 frontend feature, 7.3 settings, 7.4 empty states, 7.5 healthcheck, 7.6 i18n, 7.7 ADR-015, 7.8 DoD). **Build verde**: `pnpm install` ok, lint 5/5 verde, typecheck verde. **Risultato commerciale**: time-to-value ridotto da ~30 min (cliente naviga senza riferimenti) a ~5 min (wizard + demo) per il primo "Aha moment"; healthcheck statico abilita SLA enforceable (`docs/Commerciale/Contratto_SLA.md`).
> **Versione archivio 4.13.0:** 17 Aprile 2026 (**Sprint 5b in-repo chiuso al 100% — code-signing pre-integrato + CI completa + manuali pre-vendita**): completata l'ultima banda di automazione possibile prima dell'arrivo fisico del certificato OV Sectigo. Lato build: `apps/{agent,room-agent}/scripts/post-build.mjs` ora include funzione `signFileIfConfigured(filePath)` con detect env in ordine `CERT_PFX_PATH+CERT_PASSWORD` → `CERT_THUMBPRINT` → `CERT_SUBJECT`, timestamp server `TIMESTAMP_URL` (default `http://timestamp.sectigo.com`), skip silenzioso senza nessuna env (build dev locale Andrea identico a oggi), fail-fast con messaggio operativo se signtool manca o `.pfx` non esiste; signing applicato in sequenza ordinata copy→sign→zip→sha256 cosi' SHA256SUMS.txt riflette gli artefatti firmati che il cliente verifica; `release-licensed.bat` arricchito con step `1b/6` di preflight code-signing (rileva env CERT**, verifica `where signtool` PRIMA del build per evitare di scoprire problemi dopo 8-18 minuti di compilazione Rust). Lato CI: nuovo workflow `.github/workflows/ci.yml` con 3 jobs in matrice — `web` (Ubuntu, ~3 min: pnpm 9.15.9 + Node 22 + lint + typecheck), `agents-noFeatures` (Ubuntu, ~10 min: install webkit2gtk + gtk + librsvg2 + patchelf + cache Swatinem rust-cache + `cargo check --locked --bin {local-agent,room-agent}` no feature license), `agents-licensed` (Windows, ~15 min: cache cargo + `cargo check --locked --features license --bin ...` necessario su Windows perche' la dep `wmi` per fingerprint hardware e' Win-only e non compila su Linux); concurrency `cancel-in-progress` per non sprecare runner; `paths-ignore: docs/**, **.md`per non scattare su pure modifiche docs. Nuovo workflow`.github/workflows/playwright.yml`con trigger PR su`apps/web/**`+ push main + nightly cron`0 3 * \* \*`UTC +`workflow_dispatch`con input`run_signup_test`; setup Supabase locale via `supabase/setup-cli@v1`versione **pinned 2.20.3** (NON`latest`: rif. supabase/cli#1737 — `latest`ha rotto piu' volte`supabase start`su CI); estrae API_URL e ANON_KEY via`supabase status -o env`e li mappa nei nomi`VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY`attesi da apps/web; installa Chromium con deps + esegue`pnpm exec playwright test e2e/smoke.spec.ts --project=chromium`; upload artifact `playwright-report/`retention 7 giorni. Workflow`.github/workflows/rls-audit.yml`aggiornato con stesso pin`2.20.3`+ concurrency. Lato manuali: nuovo`docs/Manuali/Manuale_Code_Signing.md`v1.0 (10 sezioni operative per Andrea: perche' firmare con stima ROI, cosa e' gia' pronto in repo, acquisto cert OV Sectigo con reseller consigliato e workflow 7 giorni, generazione CSR via OpenSSL, installazione signtool e add to PATH, configurazione env permanente vs temporanea, tabella variabili supportate, troubleshooting 8 casi documentati, rinnovo annuale, costi totali stimati anno 1 ~253€ con ROI calcolato, checklist pre-vendita firma); nuovo`docs/Manuali/Script_Screencast.md`v1.0 con scaletta parola-per-parola dei 3 video onboarding (admin web 5-6 min + regia 4-5 min + sala 3-4 min) + setup tecnico (mic, OBS, audio -16 LUFS, branding intro/outro) + preparazione ambiente demo + checklist post-registrazione + versionamento screencast. **Architettura:** ADR-014 in`.cursor/rules/project-architecture.mdc`documenta la decisione di integrare il code-signing dentro`post-build.mjs`(non in`release-licensed.bat`) per garantire la sequenza obbligatoria sign→zip→sha256, con duplicazione tra agent (coerente con ADR-012) e pattern env-driven con skip silenzioso. **PIANO_FINALE_SLIDE_CENTER_v2.md aggiornato a v2.6** con nuova sezione 6.9 "Sprint 5b" (8 sotto-sezioni: code-signing post-build DONE, ci.yml DONE, playwright.yml DONE, pin setup-cli DONE, Manuale_Code_Signing DONE, Script_Screencast DONE, ADR-014 DONE, acceptance criteria 8/10 DONE in-repo). **Manca SOLO azione esterna Andrea (non automatizzabile dal CTO)**: (1) acquisto cert OV Sectigo (~190€/anno, emissione 1-2 settimane — guida step-by-step in `Manuale_Code_Signing.md`, settare 3 env vars dopo arrivo `.pfx`e tutto firma automaticamente); (2) registrazione 3 screencast onboarding (1 giornata — scaletta parola-per-parola in`Script_Screencast.md`); (3) revisione `Contratto_SLA.md`con avvocato GDPR + redazione DPA Allegato A (preventivo 300-800€ forfait); (4) approvazione`Listino_Prezzi.md`; (5) listing prodotti su `liveworksapp.com`. **Build verde**: `pnpm install`ok, lint e typecheck`apps/web`verdi,`cargo check --features license`e`cargo check`verdi su entrambi gli agent, dry-run`release-licensed.bat`mostra correttamente "Code-signing: SKIP" senza env settate.
**Versione archivio 4.12.0:** 17 Aprile 2026 (**Sprint 5 in-repo chiuso al 100% — hardening commerciale + materiali pre-vendita**): tutto cio' che e' automatizzabile dal CTO e' completato e committabile. Aggiunte: (a)`release-licensed.bat`orchestratore build di vendita con`cargo tauri build --features license`su entrambi gli Agent + script`build:tauri:licensed`e`release:licensed`in`apps/{agent,room-agent}/package.json`. (b) Hook NSIS `installer-hooks.nsi`su Local + Room Agent: aggiunta chiamata`<agent>.exe --deactivate`come prima istruzione di`NSIS_HOOK_PREUNINSTALL`(no-op se feature`license`non compilata, libera slot hardware sul cloud se compilata). (c)`docs/Manuali/build-pdf.ps1`automatizza conversione MD → PDF via`pandoc`+`xelatex`(fallback`wkhtmltopdf`), output in `docs/Manuali/pdf/`(gitignored), README aggiornato con`winget install Pandoc + MiKTeX`. (d) GitHub Actions `.github/workflows/rls-audit.yml`: avvia Supabase locale, applica tutte le migration via `supabase db reset --no-seed`, esegue seed minimo `supabase/tests/rls_audit_seed.sql`(2 tenant + 2 user + 1 evento + 1 sala + 1 sessione + 1 speaker + 1 presentation + 2 activity_log con UUID deterministici allineati al seed in`rls_audit.sql`), poi esegue `psql -v ON_ERROR_STOP=1 -f rls_audit.sql`(ogni`[FAIL]`blocca la PR), upload log come artifact. (e)`apps/web/scripts/upload-sourcemaps.mjs`+`postbuild`in`apps/web/package.json`: upload sourcemap a Sentry via `npx @sentry/cli@latest`, **skip silenzioso** se `SENTRY_AUTH_TOKEN`non settato (dev locali ok), errore esplicito se token presente ma`SENTRY_ORG`/`SENTRY_PROJECT`mancanti, release identifier =`slide-center-web@<pkg-version>+<git-short-sha>`, cancella `.map`da`dist/`dopo upload. (f) Bozze commerciali in nuova cartella`docs/Commerciale/`: `Contratto_SLA.md`v1.0 (10 sezioni: oggetto, attivazione + device limit, SLA cloud 99.5%, SLA desktop, sicurezza/GDPR + DPA art. 28, limitazioni responsabilita 100% canone annuo, durata + recesso, supporto P1/P2/P3/P4, IP, foro Roma legge italiana — bozza tecnica, richiede revisione legale),`Listino_Prezzi.md`v1.0 (4 piani cloud Trial/Starter/Pro/Enterprise + acquisto separato Local 490€/Room 190€ + bundle inclusi Pro/Enterprise + servizi aggiuntivi onboarding/affiancamento/sviluppo + sconti pluri-licenza/lancio/referral + esempio preventivo agenzia 8 eventi/anno + confronto competitor — bozza commerciale, richiede approvazione Andrea),`README.md` con stato bozze + schema DPA Allegato A da redigere con avvocato. (g) **Webhook Lemon Squeezy NON necessario in repo Slide Center**: gia' presente in Live WORKS APP (`functions/src/webhooks/lemonsqueezy.ts`con HMAC + eventi`order_created`+`subscription**`+ generazione license keys + idempotenza), che propaga gli eventi a Slide Center via Edge Function`licensing-sync`esistente da v4.8. Aggiungere un secondo webhook su Supabase creerebbe duplicazione, race conditions e doppio source-of-truth. **ADR-013** in`.cursor/rules/project-architecture.mdc`documenta questa scelta architetturale (singola fonte di verita Lemon → Live WORKS APP → Slide Center). (h)`.gitignore`: aggiunta `docs/Manuali/pdf/`per non committare PDF generati. **Manca SOLO azione esterna Andrea (non automatizzabile dal CTO)**: acquisto cert OV Sectigo (~190€/anno, 1-2 settimane), revisione SLA + redazione DPA art. 28 con avvocato GDPR (preventivo 300-800€ forfait), registrazione 3 screencast onboarding (~5min ciascuno), listing prodotti su`liveworksapp.com`. **Build verde**: `pnpm install`ok, lint e typecheck`apps/web`verdi,`cargo check --features license`e`cargo check`verdi su entrambi gli agent.
> **Versione archivio 4.11.0:** 17 Aprile 2026 (**Sprint 4 chiuso al 100% — sistema licenze client Tauri integrato\*\*): completata la parte client del sistema licenze centralizzato Live WORKS APP gia implementato lato cloud in v4.8. Per ogni Agent Tauri (Local + Room) creato modulo Rust completo`apps/{agent,room-agent}/src-tauri/src/license/`(7 file gemelli:`mod.rs`con`PRODUCT*ID`e chiave AES-256-GCM dedicata + diversa per agent,`types.rs`con DTO API e`LicenseStatus`enum a 7 stati,`crypto.rs`con`aes-gcm`0.10 e nonce random per ogni encrypt + 4 unit test,`fingerprint.rs`con WMI`Win32_BaseBoard.SerialNumber`+`Win32_Processor.ProcessorId`+`Win32_DiskDrive.SerialNumber`-> SHA-256 + 1 unit test,`api.rs`con`reqwest`async + user-agent dedicato per agent,`manager.rs`con orchestrazione activate/verify/deactivate + grace period 30gg + spawn_blocking per WMI,`commands.rs`con 5 comandi Tauri);`Cargo.toml`con feature flag`license`opzionale (default off per dev, on per build di vendita:`cargo tauri build --features license`); `main.rs`con early-return su flag CLI`--deactivate`(hook NSIS pre-uninstall) + registrazione condizionale dei comandi via`#[cfg(feature = "license")]`; `lib.rs`con`pub mod license`; UI `apps/{agent,room-agent}/ui/index.html`arricchita con card "Licenza" (input chiave + status pill + bottoni Attiva/Disattiva/Verifica/Copia fingerprint + dettagli cliente/scadenza), overlay full-screen di gating quando licenza non valida, polling 30s automatico per stato`pendingApproval`, JS i18n IT/EN dinamico via `navigator.language`, fallback grazioso se feature `license`non compilata (card e overlay nascosti); chiavi`license.\*`in`packages/shared/src/i18n/locales/{it,en}.json`; ADR-012 in `.cursor/rules/project-architecture.mdc`con razionale code-duplication vs shared crate, pattern di riferimento Live 3d Ledwall Render, allineamento API a`Live WORKS APP/functions/src/types/index.ts`(camelCase), acceptance criteria; manuali operatore aggiornati con sezione "Attivazione licenza" su entrambi gli installer + sezione "Build con feature license" nel manuale distribuzione;`cargo check --features license`e`cargo check` (no feature) verdi su entrambi gli agent, 10 unit test passano (`cargo test --features license --lib license::`), `pnpm lint`e`pnpm typecheck`su`apps/web`verdi.: (a) **Root**:`clean-and-build.bat`orchestratore completo (verifica toolchain Node/pnpm/cargo/cargo-tauri →`pnpm install`con fallback senza lockfile → pulizia`release/` → build Local Agent → build Room Agent → check artefatti finali con riepilogo). (b) **Local Agent (`apps/agent`)**: nuovi `package.json`(workspace`@slidecenter/agent-build`, no deps esterne — usa PowerShell `Compress-Archive`built-in),`scripts/clean.mjs`(default mantiene cache Rust per build incrementali ~1-3 min,`--full`la rimuove),`scripts/post-build.mjs`(copia NSIS in`release/live-slide-center-agent/Live-SLIDE-CENTER-Agent-Setup-0.1.0.exe`, crea ZIP portable + README dedicato, calcola `SHA256SUMS.txt`per checklist anti-tamper);`tauri.conf.json`con`bundle.targets: ["nsis"]` (rimosso MSI superfluo, dimezza tempo di build); README aggiornato. (c) **Room Agent (`apps/room-agent`)**: gemello con `@slidecenter/room-agent-build`, stessa struttura di scripts e output `release/live-slide-center-room-agent/`; README aggiornato. (d) **Documentazione operatore**: nuova cartella `docs/Manuali/`con`README.md`indice,`Manuale_Distribuzione.md`(procedura interna Andrea: toolchain, build, layout output, code-signing SmartScreen, checklist consegna licenza, troubleshooting),`Manuale_Installazione_Local_Agent.md`(operatore regia: requisiti HW, installazione, scenari cloud/intranet, troubleshooting LAN, modalita portable rescue),`Manuale_Installazione_Room_Agent.md` (operatore PC sala: discovery 4-tier, IP manuale fallback, badge connettività, troubleshooting Defender/firewall). (e) **`.gitignore`**: nuova entry `release/`per non trackare artefatti di distribuzione. Smoke test verde:`pnpm install`(6 workspace),`npm run clean`Local+Room Agent,`cargo check`Local+Room Agent (cache 1-2s), lint+typecheck`apps/web`, validazione sintattica e load script post-build. Roadmap residua (Sprint 4 client licenze + Sprint 5 hardening commerciale) tracciata in `docs/PIANO_FINALE_SLIDE_CENTER_v2.md`.
...
| 13      | Standalone-ization (era: Integrazioni ecosistema)              | **Rimossa in v4.16.1**                                 | Decisione di v4.16.1: Slide Center e' applicazione **standalone**; rimossa intera sezione "Ecosistema Live Software" da `/settings` + chiavi i18n `settings.integrations*` + env `VITE_LIVE_SPEAKER_TIMER_URL` / `VITE_LIVE_CREW_URL` + file `features/settings/lib/integrations-env.ts`. L'unico legame con la suite resta il **sistema licenze** centralizzato Live WORKS APP (ADR-012/013, unidirezionale Lemon → WORKS APP → `licensing-sync` Slide Center). Vedi ADR-011 aggiornato.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| **S6**  | Sprint 6 — Onboarding wizard + demo data + healthcheck         | **Completata in-repo (100%)** — esterne Andrea pending | **In-repo (DONE):** migration `supabase/migrations/20260417130000_onboarding_and_demo_seed.sql` con colonna `tenants.onboarded_at TIMESTAMPTZ` + 5 RPC SECURITY DEFINER (`mark_tenant_onboarded` admin self → `now()`, `reset_tenant_onboarding` admin self → NULL per "Riapri tour", `seed_demo_data` admin/coordinator idempotente con marker `settings.demo='true'`, `clear_demo_data` cancella SOLO eventi demo cascade, `tenant_health` super_admin only counter aggregati globali); `packages/shared/src/types/database.ts` esteso (Row/Insert/Update + 5 nuove RPC in `Functions`); feature `apps/web/src/features/onboarding/` con `repository.ts`, hook `useTenantOnboardingStatus.ts` (effect refactored per `react-hooks/set-state-in-effect`), `OnboardingGate.tsx` admin-only con lazy-load wizard, `components/OnboardingWizard.tsx` 3 step (Welcome + Crea evento o Demo + Finish con next-step team+agent); `OnboardingGate` montato in `RootLayout.tsx`; sezione "Demo & Onboarding" in `SettingsView.tsx` admin-only con 3 azioni `DemoActionRow` (genera demo / cancella demo / riapri tour) con stati `idle\|busy\|done\|error`; empty state CTA in `EventsView` (link a "genera demo") + `TeamView` (bottone "invita membro"); `apps/web/public/healthcheck.json` statico per UptimeRobot/BetterUptime; pagina `/admin/health` super_admin only in `apps/web/src/features/admin/AdminHealthView.tsx` (Supabase ping con badge latency colorato + Edge Functions ping con accept-401-as-online + counter `tenant_health()` in `CounterCard` + Refresh All); routing aggiornato (`routes.tsx` + `admin-root-layout.tsx` link "Health"); ~50 chiavi i18n IT+EN parity (`onboarding.*`, `settings.demo*`, `emptyState.*`, `health.*`); ADR-015 documenta scelta `tenants.onboarded_at` + auto-trigger via `OnboardingGate` + RPC SECURITY DEFINER + healthcheck doppio. Build verde: lint 5/5 + typecheck OK. **Risultato commerciale:** time-to-value ridotto da ~30 min a ~5 min per il primo "Aha moment". **Esterne Andrea (pending):** configurazione UptimeRobot puntato su `https://app.liveworksapp.com/healthcheck.json` (3 minuti); eventuale registrazione 4° screencast su "wizard onboarding". |
...
+-- supabase/
�   +-- migrations/          # Schema SQL + RLS (incl. tenant_license_sync.sql Sprint 4 cloud + Sprint 6: 20260417130000_onboarding_and_demo_seed.sql con tenants.onboarded_at + 5 RPC SECURITY DEFINER)
�   +-- functions/           # Edge Functions Deno (health, pair-init, pair-claim, pair-poll, cleanup, room-player-bootstrap, licensing-sync, team-invite-accept)
�   +-- seed.sql
�   +-- config.toml
+-- apps/web/public/healthcheck.json  # Sprint 6: endpoint statico pubblico per UptimeRobot/BetterUptime
+-- apps/web/src/features/onboarding/ # Sprint 6: repository + hook + OnboardingGate + components/OnboardingWizard (3 step modal)
+-- apps/web/src/features/admin/AdminHealthView.tsx  # Sprint 6: dashboard /admin/health (Supabase + Edge Functions ping + counter aggregati)
...
```
Live SLIDE CENTER (standalone, v4.16.1)
  +-- Licenze --> Live WORKS APP (unico canale unidirezionale Lemon → WORKS APP → licensing-sync)
                  Lemon Squeezy → Webhook WORKS APP → Trigger onLicenseChangedSyncSlideCenter
                  → Edge Function licensing-sync (HMAC) → RPC licensing_apply_quota → tenants

  (NESSUNA integrazione dati con Live Speaker Timer / Live CREW / Live PLAN.
   Sync cross-app rimossi in v4.16.1 — vedi ADR-011 aggiornato.)
```

Slide Center e' un prodotto **standalone**. L'unico legame con la suite Live Software e' il sistema licenze centralizzato (architettura unidirezionale, no sync dati operativi). Eventuali integrazioni future andranno disegnate ex-novo solo su requisiti reali emergenti dai test sul campo.
