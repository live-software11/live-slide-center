...
| 14    | Hardening + Sentry + E2E                   | **In corso (~60%)**   | Rate limit pair-claim server-side (5/15min, `pair_claim_rate_events`); RLS `current_tenant_suspended()` su tutte le tabelle operative; Sentry React lazy init (`VITE_SENTRY_DSN`); PairView stale-closure fix; Playwright dep aggiunta. **Rimangono:** audit RLS completo, Playwright E2E test suite, Sentry `captureException` in error boundary, eventuale CSP/HSTS hardening.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- [x] Fase 14 (in corso): `@sentry/react` lazy init in `init-sentry.ts`, `VITE_SENTRY_DSN` in `vite-env.d.ts` + `turbo.json`, `@playwright/test` in devDep, `pair-claim` rate limit 5/15min
