...
> **Versione:** 3.0.13 — 15 Aprile 2026 (sale: modifica inline nome/tipologia in evento; `updateRoomById`; guida §11/§15)
...
L'interfaccia tenant espone `/events` (lista + nuovo evento) e `/events/:eventId` con **Sale** (creazione, **modifica inline** nome + `room_type`, eliminazione a due passaggi), **Sessioni** (titolo, sala, `session_type`, orari `datetime-local` → UTC) e **Relatori** (sessione, nome, email opzionale). Alla creazione (o rigenerazione manuale su record legacy) il sistema assegna `upload_token` + `upload_token_expires_at` (90 giorni); in elenco compaiono **link assoluto** `/u/:token`, **copia negli appunti** e **QR** (`react-qr-code`). La pagina `/u/:token` e uno **stub** informativo fino alla Fase 3 (TUS, validazione token, Storage). Messaggi su CASCADE PostgreSQL (sala → sessioni e relatori; sessione → relatori). **Fase 2 ancora da fare:** modifica inline sessioni/relatori, calendario drag&drop, quote; **Fase 3:** upload effettivo nel portale.

**EN — Tenant UI:** Event detail supports **inline room edit** (name + room type) plus create/delete (two-step). Speakers get the **90-day upload portal URL**, **copy**, and **QR** as above; `/u/:token` remains a **placeholder** until Phase 3.

**EN:** Tenant UI exposes `/events` (list + create) and `/events/:eventId` with **Rooms** (inline edit), **Sessions**, and **Speakers** as above. **Deletion:** two-step confirm; copy explains PostgreSQL `ON DELETE CASCADE` (room → sessions and speakers; session → speakers). Phase 2+ still pending: inline edit for sessions/speakers, drag-and-drop calendar, quotas; upload portal completion in Phase 3.
...
| 2     | CRUD Eventi/Sale/Sessioni/Speaker        | **In corso**   | `/events` lista+insert; `/events/:eventId` sale (**update** nome/tipo), sessioni+relatori (lista+insert+delete conferma; link+QR invite, token 90gg; legacy rigenerabile); restano quote, import CSV, modifica sessioni/relatori, calendario |
...
**Stima MVP (fasi 1–6, cfr. logica sopra):** in aprile 2026, indicativamente **22–28%** del percorso verso il MVP cloud vendibile: fase **0** completata; fasi **1** e **2** in corso (auth, tenant, `/events`, `/events/:eventId` con sale in lista + insert + **update** nome/tipo + delete con conferma; sessioni/relatori come sopra; mancano upload TUS effettivo, regia realtime, pairing + Room Player PWA, modifica inline sessioni/relatori, quote in UI, import CSV, calendario); fasi **3–6** non iniziate. Se si considera l’intera roadmap **0–14** con pesi simili per fase, la percentuale lineare sul totale visione prodotto scende a circa **12–18%** (molte fasi ancora «Da fare» o premium).
...
**EN:** For **MVP phases 1–6**, the project is roughly **22–28%** complete (foundation + tenant event CRUD with deletes; **rooms** support inline name/type edit; major gaps: full upload/TUS portal, realtime control room, pairing + Room Player PWA, inline edit for sessions/speakers, quotas UX, CSV import, calendar). Across **all roadmap phases 0–14**, a naive equal-weight view is about **12–18%** because many phases are untouched. **Tooling:** without Docker, local Supabase CLI flows stay blocked; §18 checkboxes reflect that. **`database.ts`** stays hand-synced with migrations until `gen types --local` is viable.
