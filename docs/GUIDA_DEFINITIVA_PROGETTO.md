...
> **Versione:** 3.0.16 — 15 Aprile 2026 (import CSV relatori in dettaglio evento; `speaker-csv-import` + `importSpeakersBulk`; guida §11/§15)
...
| 2     | CRUD Eventi/Sale/Sessioni/Speaker        | **In corso**   | `/events` lista+insert+**UI quote** (storage, eventi/mese per `start_date`, gate creazione); `/events/:eventId` sale (**update** nome/tipo)+quote sale/evento+gate creazione sala; sessioni+relatori (lista+insert+**update** inline+delete conferma; link+QR invite, token 90gg; legacy rigenerabile); **import CSV relatori** (`speaker-csv-import`, max 200 righe); restano calendario drag&drop, eventuali import sale/sessioni |
...
**Stima MVP (fasi 1–6, cfr. logica sopra):** in aprile 2026, indicativamente **27–33%** del percorso verso il MVP cloud vendibile: fase **0** completata; fasi **1** e **2** in corso (auth, tenant, `/events`, `/events/:eventId` con sale in lista + insert + **update** nome/tipo + delete con conferma; sessioni/relatori con lista + insert + **update** inline + delete con conferma; **prima UI quote** letta da `tenants` + gate client su nuovo evento / nuova sala; **import CSV relatori** in dettaglio evento; mancano upload TUS effettivo, regia realtime, pairing + Room Player PWA, enforcement quote lato DB su `events`/`rooms`, calendario drag&drop); fasi **3–6** non iniziate. Se si considera l’intera roadmap **0–14** con pesi simili per fase, la percentuale lineare sul totale visione prodotto resta indicativamente **15–21%** (molte fasi ancora «Da fare» o premium).
...
**EN:** For **MVP phases 1–6**, the project is roughly **27–33%** complete (foundation + tenant event CRUD with deletes; **rooms**, **sessions**, and **speakers** support inline edits where implemented; **first quotas UX** read from `tenants` plus client-side gates for new events / new rooms; **speaker CSV import** on event detail; major gaps: full upload/TUS portal, realtime control room, pairing + Room Player PWA, DB-level quota enforcement on `events`/`rooms`, drag-and-drop calendar). Across **all roadmap phases 0–14**, a naive equal-weight view is about **15–21%** because many phases are untouched. **Tooling:** without Docker, local Supabase CLI flows stay blocked; §18 checkboxes reflect that. **`database.ts`** stays hand-synced with migrations until `gen types --local` is viable.
