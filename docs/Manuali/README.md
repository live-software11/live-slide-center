...
| `Manuale_Email_Resend.md`              | 1.0      | Andrea (build & release) | **Sprint 7**: Configurazione Resend (account, dominio, API key) + secret Supabase Edge + deploy `email-send` / `email-cron-licenses` / `gdpr-export` + schedulazione cron T-30/7/1 (GitHub Actions / cron-job.org / pg_cron) + troubleshooting. |
| `Guida_Uso_Interno_DHS.md`             | 1.0      | Andrea + team DHS        | **Sprint 8**: Quick start operativo per uso interno DHS — checklist setup una tantum, pre-evento (T-7/T-1), giorno evento, post-evento, manutenzione settimanale/mensile, troubleshooting comune, test sul campo, do/don't.                     |
