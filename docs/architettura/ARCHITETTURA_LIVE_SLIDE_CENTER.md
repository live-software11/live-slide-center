...
> **Documento UNICO di riferimento.** Questo file sostituisce e incorpora i precedenti `GUIDA_DEFINITIVA_PROGETTO.md`, `PIANO_FINALE_SLIDE_CENTER_v2.md`, `GUIDA_OPERATIVA_v3_FIELD_TEST_E_OFFLINE.md`. Per lo stato sprint corrente e le azioni pendenti vedi il documento gemello `docs/implementazioni-future/STATO_E_TODO.md`.
>
...
> - `docs/implementazioni-future/STATO_E_TODO.md` — cose da fare oggi e domani
> - `docs/operazioni/DISASTER_RECOVERY.md` — runbook emergenze + Sentry + cleanup workspace
> - `docs/operazioni/Setup_Strumenti_e_MCP.md` — setup macchina di sviluppo
> - `docs/agenti/Istruzioni_Claude_Desktop.md` — prompt di avvio AI agents (Claude Desktop + Cursor)
> - `docs/operazioni/` — manuali installazione, distribuzione, code-signing, email, Centro Slide Desktop
> - `docs/commerciali/` — SLA, listino, roadmap vendita esterna
> - `docs/archivio/` — storia sprint chiusi e doc storici (READ-ONLY)
...
Vedi `docs/operazioni/Setup_Strumenti_e_MCP.md` per setup completo macchina di sviluppo.
...
└── docs/
    ├── docs/architettura/ARCHITETTURA_LIVE_SLIDE_CENTER.md   ← QUESTO FILE (fonte di verita)
    ├── docs/implementazioni-future/STATO_E_TODO.md                      ← cose da fare
    ├── docs/operazioni/Setup_Strumenti_e_MCP.md             ← setup macchina sviluppo
    ├── docs/agenti/Istruzioni_Claude_Desktop.md         ← prompt avvio Claude Desktop
    ├── docs/operazioni/                             ← installazione, distribuzione, code-signing, email Resend
    └── docs/commerciali/                         ← SLA, listino, roadmap vendita esterna
```
...
- **Cancellazione dati**: documentata in `/settings/privacy` con riferimento a DPO (esterno, vedi `docs/commerciali/Contratto_SLA.md`).
...
Warm-keep procedure operativa per cold-start mitigation: vedi `docs/operazioni/DISASTER_RECOVERY.md` § "Appendice — Edge Functions warm-keep".
...
L'integrazione webhook GitHub → Vercel **puo' disconnettersi silenziosamente** (visto in produzione 18/04/2026 — vedi `docs/implementazioni-future/STATO_E_TODO.md` §0.26). Sintomi: Vercel dashboard mostra "Updated Nd ago" mentre `git log` ha commit recenti; `gh api repos/.../deployments` ritorna vuoto; bundle servito ha hash vecchio.
...
Procedura attivazione: vedi `docs/operazioni/Setup_Strumenti_e_MCP.md` §2c. Dettagli operativi: `.cursor/rules/mcp-vercel.mdc`.
...
Vedi `docs/implementazioni-future/STATO_E_TODO.md` §0.8 per dettaglio file modificati e azioni manuali Andrea (deploy migrations, env vars Vercel, secrets GitHub Actions).
...
| S-4    | G7  | Ruolo device "Centro Slide" multi-room (oggi: 1 device = 1 sala)                                 | **DONE** ✅ (vedi `docs/implementazioni-future/STATO_E_TODO.md` §0.15) |
...
| Fase | Nome                                                                                                                                                         | Stato |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----- |
| A    | Cloud finale: rigenerati tipi Supabase (rimossi `unknown as` + `rpcLoose()`), unificata UX cartelle nel solo File Explorer V2, daily backup verifier (PowerShell + GH Actions cron), nuovo `docs/operazioni/DISASTER_RECOVERY.md` runbook 5 scenari | DONE  |
...
| Item                                | Nome                                                                                                                                  | Stato |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| Sentry runtime monitoring           | Setup org `live-work-app` (region EU `de.sentry.io`) + project `live-slide-center-web`. Init lazy in `apps/web/src/lib/init-sentry.ts` con `tracesSampleRate=0.1`, telemetry helper `apps/web/src/lib/telemetry.ts`, ErrorBoundary integrato. Vedi `docs/operazioni/DISASTER_RECOVERY.md` § Setup Sentry | DONE  |
...
| Documentazione consolidata          | 29 doc → ~14 doc canonici + `docs/archivio/`. Merge Setup_PC_Centro_Slide.md + Smoke_Test_Centro_Slide.md → `Manuale_Centro_Slide_Desktop.md`. Merge EDGE_FUNCTIONS_WARM_KEEP.md → `docs/operazioni/DISASTER_RECOVERY.md` appendice. docs/implementazioni-future/STATO_E_TODO.md snellito 304→37 KB (-88%). Nuovo `docs/README.md` indice | DONE  |

#### Sprint X-1 — Upload hardening (desktop + cloud + smoke) — DONE 19/04/2026 sera

Audit chirurgico in 3 punti scoperto durante la review desktop e fixato in 3 commit isolati prima del primo field test reale:

| Punto | File | Bug | Fix |
| ----- | ---- | --- | --- |
| **A** Desktop upload | `apps/web/src/features/upload-portal/lib/simple-upload.ts` (NUOVO) + `apps/web/src/features/presentations/hooks/useUploadQueue.ts` + `apps/web/src/features/presentations/components/AdminUploaderInline.tsx` | La SPA in modalita desktop usava `tus-js-client` contro il server Rust Axum locale che NON implementa il protocollo TUS (solo `POST /storage/v1/object/{bucket}/{*key}` diretto, vedi `apps/desktop/src-tauri/src/server/routes/storage_routes.rs`). Risultato: upload sempre rotti in desktop, l'utente non poteva caricare nulla via UI. | Nuovo `simple-upload.ts` con `XMLHttpRequest` (per progress events) + branching in `useUploadQueue.ts` / `AdminUploaderInline.tsx` su `getBackendMode()`: `desktop` → `startSimpleUpload({baseUrl, adminToken, bucket, objectName, file})`, `cloud` → `startTusUpload({supabaseUrl, anonKey, accessToken, ...})`. Stessa firma `UploadHandle` (`{abort()}`) per omogeneita. Smoke test E2E nuovo `apps/desktop/scripts/smoke-test-upload.mjs` (RPC init → POST → SHA-256 → finalize → verify `current_version_id` + filesystem locale → cleanup) lanciabile con `pnpm --filter @slidecenter/desktop smoke-upload`. |
| **B** Cloud TUS race-cancel | `apps/web/src/features/presentations/hooks/useUploadQueue.ts` + `apps/web/src/features/presentations/components/AdminUploaderInline.tsx` | In modalita cloud `getSession()` veniva chiamata in `.then()` dentro un Promise constructor. Se l'utente cancellava il job durante l'await asincrono di `getSession()`, il callback `.then()` partiva comunque chiamando `startTusUpload(...)` → bandwidth sprecata (potenzialmente 100MB+ su 4G), `abort()` non poteva intercettare un handle ancora `null`, finalize procedeva su upload non desiderato → orphan files in `presentation_versions` con stato `uploading` mai chiusi. | In `useUploadQueue.ts`: aggiunto `if (job.cancelled) { reject(new Error('upload_cancelled')); return; }` come prima istruzione del `.then(({data: sessionData}) => ...)`, sfruttando `job.cancelled = true` settato da `cancel()` (che chiama anche `abortAdminUpload(versionId)` server-side). In `AdminUploaderInline.tsx`: nuovo `uploadCancelledRef = useRef(false)` settato a `true` da `cancelEverything()` e resettato a `false` all'inizio di `startUpload()`, check in 2 finestre: (1) dentro `.then()` di `getSession()` prima di `startTusUpload`, (2) dopo `await uploadPromise` ma prima di hashing/finalization (chiama `abortAdminUpload(init.version_id)` per pulire la version). Il catch upper riconosce `Error('upload_cancelled')` e ritorna early senza marcare lo stato come `error` (preserva l'`idle` settato da `onCancel`). |
| **C** Smoke test secrets | `apps/web/scripts/smoke-test-upload.mjs` | Lo script aveva hardcoded Supabase URL, anon key, email field-test e password in chiaro. Anche se scoped a un tenant di test, committarle in repo creava esposizione (history + fork + PR pubbliche). | Rimossi tutti i valori hardcoded. Aggiunti helpers `readArgOrEnv(flagName, envName)` + `requireOrFail(value, hint)`: lo script ora richiede CLI flags `--supabase-url`, `--anon-key`, `--email`, `--password`, `--session-id` OR env vars `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `SC_SMOKE_EMAIL`, `SC_SMOKE_PASSWORD`, `SC_SMOKE_SESSION_ID`. Fail-fast con messaggio chiaro se mancanti. Field test: pattern + email in `docs/operazioni/FIELD_TEST_CREDENTIALS.md` (niente tabella password; niente `service_role` in git). |
| **D** Storage RLS upload-portal | `supabase/migrations/20260419093026_sprint_x1_fix_admin_upload_storage_rls.sql` | Le precedenti policy `anon_insert_uploading_version` e `tenant_insert_uploading_version` su `storage.objects` usavano subquery direct contro `presentation_versions` che falliva con HTTP 403 quando l'admin autenticato passava `anon_key` come Bearer per TUS (il subquery girava nel ruolo `anon` senza visibilita su `presentation_versions` filtrate per tenant). | Refactor su 2 funzioni `SECURITY DEFINER`: `public.storage_can_upload_object_anon(p_object_name text)` (controlla solo `pv.status = 'uploading'`) + `public.storage_can_upload_object_tenant(p_object_name text)` (in piu', match `pv.tenant_id = public.app_tenant_id()`). Le policy su `storage.objects` ora chiamano queste funzioni invece di fare subquery raw. Funziona sia per upload pubblici da Upload Portal (anon) sia per admin authenticated. Migration applicata in cloud `cdjxxxkrhgdkcpkkozdl` il 19/04/2026 (verificato via MCP `list_migrations`). |

**Quality gates Sprint X-1:** typecheck `pnpm --filter @slidecenter/web typecheck` verde, `pnpm --filter @slidecenter/web build` verde (~8s, ~2700 modules), Vercel ready, smoke cloud verde post-deploy.

**Impatto operativo:** prima del primo field test reale (vedi `docs/operazioni/FIELD_TEST_CHECKLIST.md` T9 "Upload speaker via QR" + T6 "Pairing PC desktop server") tutti e 3 i path di upload sono coperti: upload portal anonimo (TUS cloud), admin authenticated (TUS cloud + RLS via SECURITY DEFINER), admin desktop (POST diretto Rust). La cancel UI in cloud non lascia piu' orfani.

#### Sprint X-2 — Field-test hotfix (TUS abort + ES256 edge function + cache PWA) — DONE 19/04/2026 sera tardi

Tre errori distinti emersi durante il primo field test reale di Andrea, fixati nello stesso giorno prima del rientro in evento:

| Punto | File | Bug | Fix |
| ----- | ---- | --- | --- |
| **A** TUS DELETE 403 | `apps/web/src/features/presentations/hooks/useUploadQueue.ts` + `apps/web/src/features/presentations/components/AdminUploaderInline.tsx` | Quando un upload TUS chiudeva con `status='done'`, lo `useEffect` cleanup dell'unmount chiamava comunque `uploadHandle.abort()`. `tus-js-client` traduce `abort()` in HTTP `DELETE /storage/v1/upload/resumable/<key>` per pulire la sessione resumable, ma il file finalizzato e' gia' INSERT-ato in `storage.objects` quindi la nuova policy RLS lo blocca con 403 "new row violates row-level security policy". Effetto utente: console error rumoroso, e in alcune race condition orphan file lock. | Introdotto helper `updateTerminal(patch)` in `useUploadQueue.ts` e `terminalState(next)` in `AdminUploaderInline.tsx` che nullano `uploadHandle`/`hashAbort`/`uploadHandleRef` PRIMA di settare lo stato a `done`/`error`/`cancelled`. Lo `useEffect` cleanup ora skippa job in stato terminale (`if (j.status === 'done' || 'error' || 'cancelled') continue;`) e usa `?.abort()` come safety net se per qualche ragione il ref non e' stato nullato. Niente piu' DELETE 403 spuria. |
| **B** Edge function 401 ES256 | `supabase/config.toml` + redeploy `slide-validator` v3 | Da Supabase 2025+ le JWT signing keys sono ES256 (asimmetriche) per default. Il legacy verifier del platform gateway (`verify_jwt = true` nel TOML) NON supporta ES256 e rifiutava token validi con HTTP 401 PRIMA di invocare il codice della function. Effetto: `slide-validator` non veniva mai eseguita, nessun warning di validazione popolato (font non embedded, PDF corrotti, ecc.). | Cambiato `verify_jwt = true` → `false` nella section `[functions.slide-validator]` del `config.toml`, **non** abbassando la sicurezza: la function continua a verificare il JWT internamente via `getAuthenticatedUser(req)` (`_shared/auth.ts`) che usa `admin.auth.getUser(jwt)` con service-role, path che supporta ES256 correttamente e ritorna 401 esplicito se il token e' invalido/scaduto. Il check `tenant_id` su `presentation_versions` resta intatto. Stesso pattern gia' usato per `pair-init` e `pair-poll`. Edge function v3 deployata in cloud `cdjxxxkrhgdkcpkkozdl` il 19/04/2026 via MCP `deploy_edge_function`. |
| **C** PWA cache stale (`__all__` UUID) | (no code change — root cause su client di Andrea) | Andrea ha visto ricomparire l'errore `invalid input syntax for type uuid: "__all__"` su `move_presentations_to_folder`. Il fix `folderIdFromSelection()` in `ProductionView.tsx` era gia' deployato, ma il service worker della PWA serviva il chunk `ProductionView-*.js` precedente da cache. | Nessuna modifica codice (gia' presente `vite-plugin-pwa` con `registerType: 'autoUpdate'` + `skipWaiting: true` + `clientsClaim: true` in `apps/web/vite.config.ts`, listener `controllerchange` → `window.location.reload()` in `apps/web/src/main.tsx`). Soluzione operativa: hard reload (Ctrl+Shift+R) sul tablet/PC dell'utente, oppure attendere il successivo `controllerchange`. Documentato in `docs/operazioni/FIELD_TEST_CHECKLIST.md` "Pre-flight"). |

**Quality gates Sprint X-2:** typecheck verde, lint verde, `pnpm --filter @slidecenter/web build` verde (~8s, ~2700 modules), edge function deploy v3 ACTIVE (`verify_jwt: false`), Vercel deploy ready.
...
| 022 | Upload TUS solo in cloud, POST diretto in desktop (Sprint X-1)       | Server Rust locale non implementa TUS (overkill per LAN). Branching in `useUploadQueue` su `getBackendMode()`. Stessa interfaccia `UploadHandle.abort()` per UX identica |
| 023 | RLS storage upload via SECURITY DEFINER functions (Sprint X-1)       | Subquery raw fallivano con `anon` Bearer durante TUS. Funzioni `storage_can_upload_object_anon/_tenant` isolano la logica e funzionano sia per upload portal anonimo sia per admin authenticated |
| 024 | Smoke test cloud SOLO via env vars / CLI flags (Sprint X-1)          | Mai hardcodare credenziali in repo, anche se scoped a field test. Pattern + email in `docs/operazioni/FIELD_TEST_CREDENTIALS.md` (niente tabella password, niente `service_role`) |
...
**FINE.** Per cose da fare oggi e domani: vedi `docs/implementazioni-future/STATO_E_TODO.md`. Per setup macchina: `docs/operazioni/Setup_Strumenti_e_MCP.md`. Per prompt Claude Desktop: `docs/agenti/Istruzioni_Claude_Desktop.md`.
