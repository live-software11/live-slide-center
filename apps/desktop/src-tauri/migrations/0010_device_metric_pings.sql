...
CREATE TABLE IF NOT EXISTS device_metric_pings (
  ...
  event_id TEXT REFERENCES events(id) ON DELETE
  SET NULL,
    room_id TEXT REFERENCES rooms(id) ON DELETE
  SET NULL,
    ...
    visibility TEXT CHECK (
      visibility IS NULL
      OR visibility IN ('visible', 'hidden')
    ),
    cpu_pct REAL,
    ram_used_pct REAL,
    ram_used_mb REAL,
    ...
    playback_mode TEXT CHECK (
      playback_mode IS NULL
      OR playback_mode IN ('auto', 'live', 'turbo')
    ),
    device_role TEXT CHECK (
      device_role IS NULL
      OR device_role IN ('room', 'control_center')
    ),
    CHECK (
      (
        js_heap_used_pct IS NULL
        OR (
          js_heap_used_pct >= 0
          AND js_heap_used_pct <= 100
        )
      )
      AND (
        storage_quota_used_pct IS NULL
        OR (
          storage_quota_used_pct >= 0
          AND storage_quota_used_pct <= 100
        )
      )
      AND (
        cpu_pct IS NULL
      ...
      AND (
        ram_used_pct IS NULL
        OR (
          ram_used_pct >= 0
          AND ram_used_pct <= 100
        )
      )
      AND (
        disk_free_pct IS NULL
        OR (
          disk_free_pct >= 0
          AND disk_free_pct <= 100
        )
      )
      AND (
        battery_pct IS NULL
...
CREATE INDEX IF NOT EXISTS idx_device_metric_pings_device_ts ON device_metric_pings(device_id, ts DESC);
CREATE INDEX IF NOT EXISTS idx_device_metric_pings_event_ts ON device_metric_pings(event_id, ts DESC)
WHERE event_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_device_metric_pings_ts ON device_metric_pings(ts);
