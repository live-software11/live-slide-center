...
CREATE TABLE IF NOT EXISTS event_folders (
  ...
  created_by TEXT REFERENCES users(id) ON DELETE
  SET NULL
);
...
CREATE INDEX IF NOT EXISTS idx_event_folders_parent ON event_folders(parent_id)
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_event_folders_name_per_parent ON event_folders (
  event_id,
  COALESCE(
    parent_id,
...
ADD COLUMN folder_id TEXT REFERENCES event_folders(id) ON DELETE
SET NULL;
