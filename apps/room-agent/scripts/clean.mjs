#!/usr/bin/env node
/**
 * Pulizia artefatti di build per Live SLIDE CENTER Room Agent.
 *
 * Default: rimuove SOLO il bundle finale (riusa la cache di compilazione Rust → build incrementale).
 *   - src-tauri/target/release/bundle
 *   - src-tauri/target/release/room-agent.exe
 *
...
const targets = full
  ...
  : [
    ...
    join(root, 'src-tauri', 'target', 'release', 'room-agent.exe'),
    join(root, 'src-tauri', 'target', 'release', 'room-agent.pdb'),
  ];
