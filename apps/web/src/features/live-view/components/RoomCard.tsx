...
export function RoomCard({ data }: Props) {
  ...
  const barColor =
    uploadRatio >= 1 ? 'bg-emerald-500' : uploadRatio > 0.5 ? 'bg-blue-500' : uploadRatio > 0 ? 'bg-amber-500' : 'bg-zinc-700';

  return (
    <div className="flex flex-col rounded-lg border border-zinc-800 bg-[#141416] p-4">
      <header className="mb-3 flex items-center justify-between gap-2">
        ...
        <span className="shrink-0 text-xs font-mono tabular-nums text-zinc-400">
      ...
      {currentSession ? (
        <div className="mb-2 rounded-md border border-blue-900/40 bg-blue-950/30 px-3 py-2">
          <p className="text-[10px] uppercase tracking-wide text-blue-400">{t('liveView.nowPlaying')}</p>
          <p className="mt-0.5 text-sm font-medium text-zinc-100 line-clamp-1">{currentSession.title}</p>
          <p className="text-xs text-zinc-400">
            {timeFmt.format(new Date(currentSession.scheduled_start))} – {timeFmt.format(new Date(currentSession.scheduled_end))}
          </p>
        </div>
      ) : (
        <div className="mb-2 rounded-md border border-zinc-800 bg-zinc-900/40 px-3 py-2">
          <p className="text-[10px] text-zinc-500">{t('liveView.noCurrentSession')}</p>
        </div>
      )}

      {nextSession && (!currentSession || nextSession.id !== currentSession.id) ? (
        <div className="mb-3 rounded-md border border-zinc-800 bg-zinc-900/40 px-3 py-1.5">
          <p className="text-[10px] uppercase tracking-wide text-zinc-500">{t('liveView.upNext')}</p>
          <p className="text-xs text-zinc-300 line-clamp-1">{nextSession.title}</p>
          ...
          {speakers.slice(0, 6).map((sp) => {
            ...
            return (
              <li key={sp.id} className="flex items-center gap-2 text-xs">
                ...
                />
                <span className="truncate text-zinc-300">{sp.full_name}</span>
              </li>
            );
          })}
          {speakers.length > 6 ? (
