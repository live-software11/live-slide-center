-- Quote storage + default Trial come da docs/GUIDA_DEFINITIVA_PROGETTO.md §7

CREATE OR REPLACE FUNCTION public.check_storage_quota() RETURNS TRIGGER
LANGUAGE plpgsql
...
BEGIN
  ...
  IF v_limit >= 0 AND (v_used + NEW.file_size_bytes) > v_limit THEN
...
CREATE TRIGGER track_storage_used
  AFTER INSERT OR DELETE ON presentation_versions
