...
OR p_token = '' THEN RAISE EXCEPTION 'invalid_token' USING ERRCODE = 'check_violation';
END IF;
...
SELECT id,
  ...
  room_id,
  status,
  device_name INTO v_device
FROM paired_devices
WHERE pair_token_hash = v_hash
LIMIT 1;
IF NOT FOUND THEN RAISE EXCEPTION 'device_not_found' USING ERRCODE = 'check_violation';
END IF;
...
IF v_device.status = 'offline' THEN RAISE EXCEPTION 'device_offline' USING ERRCODE = 'insufficient_privilege';
END IF;
IF v_device.room_id IS NULL THEN RAISE EXCEPTION 'device_no_room_assigned' USING ERRCODE = 'check_violation';
END IF;
SELECT suspended INTO v_tenant_suspended
FROM tenants
WHERE id = v_device.tenant_id;
IF v_tenant_suspended THEN RAISE EXCEPTION 'tenant_suspended' USING ERRCODE = 'insufficient_privilege';
END IF;
...
SELECT s.id,
  s.event_id,
  s.tenant_id,
  s.room_id INTO v_session
FROM sessions s
WHERE s.id = p_session_id
  AND s.tenant_id = v_device.tenant_id
LIMIT 1;
IF NOT FOUND THEN RAISE EXCEPTION 'session_not_found_or_cross_tenant' USING ERRCODE = 'check_violation';
END IF;
IF v_session.room_id IS DISTINCT
FROM v_device.room_id THEN RAISE EXCEPTION 'session_cross_room_not_allowed' USING ERRCODE = 'check_violation';
END IF;
IF v_session.event_id IS DISTINCT
FROM v_device.event_id THEN RAISE EXCEPTION 'session_cross_event_not_allowed' USING ERRCODE = 'check_violation';
END IF;
...
END;
$$;
REVOKE ALL ON FUNCTION public.init_upload_version_for_room_device(text, uuid, text, bigint, text)
FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.init_upload_version_for_room_device(text, uuid, text, bigint, text) TO service_role;
COMMENT ON FUNCTION public.init_upload_version_for_room_device IS 'Sprint R-3 + audit-fix 2026-04-19: PC sala (auth via device_token hash) avvia upload nuova versione su sessione della propria sala. Rifiuta se device.status=offline (revoca esplicita o disconnessione). Ritorna metadata per signed upload URL Storage.';
