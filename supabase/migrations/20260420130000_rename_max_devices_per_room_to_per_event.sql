...
DROP FUNCTION IF EXISTS public.licensing_apply_quota(text, uuid, tenant_plan, bigint, integer, integer, timestamp with time zone, text, integer);
DROP FUNCTION IF EXISTS public.licensing_apply_quota(text, uuid, tenant_plan, bigint, integer, integer, timestamp with time zone, text, integer, integer);
...
REVOKE ALL ON FUNCTION public.licensing_apply_quota(
  text, uuid, tenant_plan, bigint, integer, timestamp with time zone, text, integer, integer, integer
) FROM PUBLIC;
...
-- ── 4) Helper SQL nuova ────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.tenant_max_devices_per_event(p_tenant_id uuid)
RETURNS INTEGER LANGUAGE sql STABLE
SET search_path = public AS $$
  SELECT max_devices_per_event FROM tenants WHERE id = p_tenant_id;
$$;
