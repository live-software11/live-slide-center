...
-- Coerente con docs/GUIDA_DEFINITIVA_PROGETTO.md §6 (Auth) — super_admin solo via SQL manuale §10.
...
BEGIN
  ...
  VALUES (NEW.id, new_tenant_id, NEW.email, org_name, 'admin'::public.user_role);

  UPDATE auth.users
  SET raw_app_meta_data =
    COALESCE(raw_app_meta_data, '{}'::jsonb)
    || jsonb_build_object('tenant_id', new_tenant_id::text, 'role', 'admin')
  WHERE id = NEW.id;
