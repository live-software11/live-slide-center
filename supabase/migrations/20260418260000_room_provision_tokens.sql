...
CREATE INDEX IF NOT EXISTS idx_room_provision_tokens_expires ON public.room_provision_tokens(expires_at)
WHERE revoked_at IS NULL
  AND consumed_count < max_uses;
-- Index per lookup admin "lista token attivi della mia sala/evento".
CREATE INDEX IF NOT EXISTS idx_room_provision_tokens_room ON public.room_provision_tokens(room_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_room_provision_tokens_event ON public.room_provision_tokens(event_id, created_at DESC);
...
CREATE POLICY tenant_isolation ON public.room_provision_tokens FOR ALL USING (tenant_id = public.app_tenant_id());
-- Super-admin (debug platform) vede tutti.
DROP POLICY IF EXISTS super_admin_all ON public.room_provision_tokens;
CREATE POLICY super_admin_all ON public.room_provision_tokens FOR ALL USING (public.is_super_admin());
GRANT SELECT,
  INSERT,
  UPDATE,
  DELETE ON public.room_provision_tokens TO authenticated;
COMMENT ON TABLE public.room_provision_tokens IS 'Sprint U-4: magic-link tokens per zero-friction provisioning di PC sala. Token plain mai persistito (sha256 in token_hash).';
-- ============================================================================
...
OR p_room_id IS NULL THEN RAISE EXCEPTION 'missing_required_args' USING ERRCODE = 'check_violation';
END IF;
...
IF v_minutes < 5 THEN v_minutes := 5;
END IF;
IF v_minutes > 43200 THEN v_minutes := 43200;
END IF;
v_max_uses := COALESCE(p_max_uses, 1);
IF v_max_uses < 1 THEN v_max_uses := 1;
END IF;
IF v_max_uses > 10 THEN v_max_uses := 10;
END IF;
...
IF v_tenant_id IS NULL THEN RAISE EXCEPTION 'room_not_found_or_forbidden' USING ERRCODE = 'check_violation';
END IF;
...
v_token_plain := replace(
  replace(replace(v_token_plain, '+', '-'), '/', '_'),
  '=',
  ''
);
v_token_hash := encode(
  extensions.digest(v_token_plain, 'sha256'),
  'hex'
);
...
END;
$$;
REVOKE ALL ON FUNCTION public.rpc_admin_create_room_provision_token(uuid, uuid, integer, integer, text)
FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.rpc_admin_create_room_provision_token(uuid, uuid, integer, integer, text) TO authenticated;
COMMENT ON FUNCTION public.rpc_admin_create_room_provision_token(uuid, uuid, integer, integer, text) IS 'Sprint U-4: genera magic-link token per provisioning PC sala. Ritorna token plain una volta sola; in DB solo sha256.';
-- ============================================================================
...
IF NOT FOUND THEN -- Distinguiamo i casi (UI piu' chiara per l'operatore di sala):
...
PERFORM 1
FROM public.room_provision_tokens
WHERE token_hash = v_token_hash
LIMIT 1;
...
-- l'hash sha256.
--
  -- Audit-fix Sprint U-5+1 (X1): wrap in BEGIN/EXCEPTION per catturare
-- `unique_violation` su `paired_devices.pair_token_hash` (UNIQUE constraint
-- definito in 20250415120000_pairing_super_admin.sql). Casi possibili:
--   - bug client che riusa lo stesso pair_token su due claim consecutivi
--     (es. cabina + regia con max_uses=2 e applicativo che cache-a male)
--   - collisione SHA-256 di token random (probabilita' ~ 0 ma per
--     completezza)
-- Senza questo handler il client riceverebbe un errore Postgres tecnico
-- generico ("duplicate key value violates ..."), che l'edge function
-- classifica come 400 default e il client non puo' gestire semanticamente.
-- Ora alziamo `pair_token_collision` esplicito → edge function ritorna 409
-- e il client sa che deve rigenerare il pair_token e riprovare.
-- NB: `consumed_count` e' gia' stato incrementato sopra. L'eccezione
-- propagata fa rollback dell'intera transazione PL/pgSQL → consumed_count
-- torna al valore originale (atomicita' garantita da PostgreSQL).
BEGIN
...
EXCEPTION
WHEN unique_violation THEN RAISE EXCEPTION 'pair_token_collision' USING ERRCODE = 'unique_violation';
END;
-- Activity log
BEGIN
INSERT INTO public.activity_log (
...
VALUES (
    ...
    jsonb_build_object(
      ...
      v_device_id,
      'consumed_count',
      v_provision.consumed_count,
      'max_uses',
      v_provision.max_uses
    )
  );
EXCEPTION
WHEN OTHERS THEN NULL;
...
END;
$$;
REVOKE ALL ON FUNCTION public.rpc_consume_room_provision_token(text, text, text, text, text, text, text)
FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.rpc_consume_room_provision_token(text, text, text, text, text, text, text) TO service_role;
COMMENT ON FUNCTION public.rpc_consume_room_provision_token(text, text, text, text, text, text, text) IS 'Sprint U-4: consume di magic-link token (chiamata da edge function room-provision-claim). Crea paired_devices atomicamente.';
-- ============================================================================
-- 4. RPC admin: revoca token attivo
-- ============================================================================
CREATE OR REPLACE FUNCTION public.rpc_admin_revoke_room_provision_token(p_token_id UUID) RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER
SET search_path = public AS $$
...
RETURNING id,
  tenant_id,
  event_id,
  room_id INTO v_record;
IF NOT FOUND THEN RAISE EXCEPTION 'token_not_found_or_already_revoked' USING ERRCODE = 'check_violation';
END IF;
BEGIN
INSERT INTO public.activity_log (
    tenant_id,
...
END;
$$;
REVOKE ALL ON FUNCTION public.rpc_admin_revoke_room_provision_token(uuid)
FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.rpc_admin_revoke_room_provision_token(uuid) TO authenticated;
COMMENT ON FUNCTION public.rpc_admin_revoke_room_provision_token(uuid) IS 'Sprint U-4: revoca un magic-link token attivo (admin-only).';
